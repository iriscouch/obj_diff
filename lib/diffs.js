...
function make_diff(from, to) {
  if(DEFS.couchdb) {
    from = JSON.parse(JSON.stringify(from || {})); // Make a copy because this will be modified to force certain matches.
    from._revisions = {};
  }
...
Diff.prototype.nochange = DEFS.assert ? assert_nochange : nochange;
...
function assert_nochange() {
  var self = this;
  var result = nochange.apply(self, arguments);
  assert.equal(result, true, 'Must not change');
}

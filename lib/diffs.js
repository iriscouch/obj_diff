...
function make_diff(from, to) {
  ...
  //lib.log('make_diff: ' + lib.I({from:from, to:to}))
...
function obj_diff(from, to, result, prefix, is_array) {
  ...
  lib.log('doc_diff: from=%j to=%j', from, to);
  result = result || {};
  ...
  all_keys.forEach(function(key) {
    ...
    lib.dir({toprop:to.hasOwnProperty(key), fromprop:from.hasOwnProperty(key)});
...
function assert_atmost() {
  ...
  if(first) {
    var msg = 'Invalid change: "' + first.key + '": ' + lib.I(first.from) + ' -> ' + lib.I(first.to);

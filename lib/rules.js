...
Rule.prototype.match = function(key, from, to) {
  ...
  lib.log('Match ' + lib.I([self.key, self.from, self.to]) + ' to ' + lib.I([key, from, to]));
...
function element_match(guide, element, other) {
  ...
  lib.log('element match: ' + lib.I(guide) + ' to ' + lib.I([element, other]));
  lib.log(' => ' + result);
  return result;
}
...
function _element_match(guide, element, other) {
  ...
  lib.log(lib.I({el:element, gu:guide, ot:other}));
...
function make_rules(with_reason, args) {
  ...
  lib.log('make_rules ('+with_reason+'): args=' + JSON.stringify(args))
